...
void SPI_Speed_Test(void)
{
    ...
    LCD_Clear(WHITE);
    POINT_COLOR = BLACK;
    LCD_ShowString(10, 10, 200, 16, 16, (uint8_t*)"SPI Speed Test");
    
    // 测试清屏速度
    LCD_Clear(RED);
    LCD_ShowString(10, 40, 200, 16, 16, (uint8_t*)"Clear Screen OK");
    
    // 测试绘制矩形速度
    for(i = 0; i < 10; i++)
    {
        LCD_Fill(10 + i*20, 70, 25 + i*20, 90, BLUE);
        delay_ms(50);
    }
    
    LCD_ShowString(10, 100, 200, 16, 16, (uint8_t*)"Draw Rect OK");
    
    // 测试文字显示速度
    for(i = 0; i < 5; i++)
    {
        LCD_ShowString(10, 130 + i*20, 200, 16, 16, (uint8_t*)"Text Line");
        delay_ms(100);
    }
    
    LCD_ShowString(10, 250, 200, 16, 16, (uint8_t*)"Speed Test Complete!");
}
