...
{	 		  	  
	// 简单的延时函数，可以根据需要调整
	volatile uint32_t delay = nms * 8000;
	while (delay--);
} 
void LCD_WR_REG(uint16_t data)
{
		GPIO_ResetBit(SPI_CS_GPIO_PORT, SPI_CS_PIN);  // CS = 0
		GPIO_ResetBit(SPI_DC_GPIO_PORT, SPI_DC_PIN);  // DC = 0 (写命令)
		
		// 等待发送缓冲区空
		while(SPI_I2S_ReadStatusFlag(LCD_SPI, SPI_FLAG_TXBE) == RESET);
		// 发送数据
		SPI_I2S_TxData(LCD_SPI, data);
		// 等待发送完成
...
void LCD_Init(void)
{
	...
	// 配置CS引脚为输出
	gpioConfig.pin = SPI_CS_PIN;
	gpioConfig.mode = GPIO_MODE_OUT_PP;
	gpioConfig.speed = GPIO_SPEED_50MHz;
	GPIO_Config(SPI_CS_GPIO_PORT, &gpioConfig);
	
	// 配置DC引脚为输出
	gpioConfig.pin = SPI_DC_PIN;
	GPIO_Config(SPI_DC_GPIO_PORT, &gpioConfig);
	
	// 配置RST引脚为输出
	gpioConfig.pin = SPI_RST_PIN;
	GPIO_Config(SPI_RST_GPIO_PORT, &gpioConfig);
	...
	// MISO (PB14) - 如果需要读取数据
	gpioConfig.pin = SPI_SDO_PIN;
	gpioConfig.mode = GPIO_MODE_IN_FLOATING;
	GPIO_Config(SPI_SDO_GPIO_PORT, &gpioConfig);
	
	// 初始化引脚状态
	GPIO_SetBit(SPI_CS_GPIO_PORT, SPI_CS_PIN);  // CS高电平
	GPIO_SetBit(SPI_DC_GPIO_PORT, SPI_DC_PIN);  // DC高电平
	...
	delay_ms(1);
	GPIO_ResetBit(SPI_RST_GPIO_PORT, SPI_RST_PIN);
	delay_ms(10);
	GPIO_SetBit(SPI_RST_GPIO_PORT, SPI_RST_PIN);
	delay_ms(120);  
...
static uint8_t spi_read_byte(void)
{
    ...
    // 发送一个空字节来产生时钟
    while(SPI_I2S_ReadStatusFlag(LCD_SPI, SPI_FLAG_TXBE) == RESET);
    SPI_I2S_TxData(LCD_SPI, 0x00);
    
    // 等待接收数据
    while(SPI_I2S_ReadStatusFlag(LCD_SPI, SPI_FLAG_RXBNE) == RESET);
    data = SPI_I2S_RxData(LCD_SPI);
...
uint32_t LCD_Read_ID(void)
{
    ...
    /* 进入数据阶段（DC = 1），连续读 3 字节 */
    GPIO_SetBit(SPI_DC_GPIO_PORT, SPI_DC_PIN);    // DC = 1 数据

    id[0] = spi_read_byte();   /* dummy 0x00 */
    id[1] = spi_read_byte();   /* manufacturer ID，通常是 0x85 */
